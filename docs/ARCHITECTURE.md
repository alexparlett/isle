...
A password manager, a VPN, a snapshot tool, a phone bridge or a cloud
drive is a provider: a folder under `~/.config/isle/vaults/<name>/`,
`~/.config/isle/vpns/<name>/`, `~/.config/isle/snapshots/<name>/` or
`~/.config/isle/phones/<name>/` or `~/.config/isle/drives/<name>/` holding
`provider.json` and a script (the shell's own live under `shell/vaults`,
`shell/vpns`, `shell/snapshots`, `shell/phones` and `shell/drives`). A
manifest may carry `args`, passed to its script before the command, so one
implementation can serve several providers: every cloud drive is the same
script told which service it is. The manifest:
...
| display recovery | A monitor handed back by a KVM or a cable is not always driven again: the compositor keeps the connector up with a CRTC, never re-probes it, and the screen stays dark while the session paints happily into it. `Displays.redetect` releases every output (`hl.monitor{ disabled = true }`) and reloads the config, which re-probes and re-acquires them; `force_renderer_reload` does not. It is one shell command, so a half-done sequence cannot leave the session screenless, and it has a keymap action (Super+Ctrl+D) because the screen it fixes cannot be seen. Every output gone also leaves the compositor on its headless `FALLBACK` monitor, which `Displays.stranded` notices and heals with a reload two seconds later. IPC `displays redetect | status` (D56) |
...
| drives | `Drives` fronts the cloud storage services, one provider per service over rclone (`shell/drives`, `~/.config/isle/drives/<name>/provider.json`), each manifest naming the rclone backend it is and all running `scripts/drives.py`, so another service is a manifest rather than code. Settings › Drives lists the accounts, not the services: Add account opens a sheet whose fields come from `rclone config providers`, so Proton asks for a username, password and 2FA while Google, Dropbox and OneDrive are a name and a consent page in a terminal. An account is a filesystem of the shell's own, `scripts/drivefs`, run by `isle-drive@<name>.service` and mounted at `~/Drives/<service>`: listings and attributes come from a SQLite index, contents from a private rclone mount under the runtime directory, and every call down has a deadline. `scripts/drivectl.py` is what the file managers' menus and Settings ask; the GTK ones read each file's state from `metadata::emblems` and Dolphin from `plugins/isle-overlay`; `scripts/drivemon.py` with `system/gvfs/isle.monitor` would name it in the sidebar, which the mount point's own name does for now. IPC `drives status | services | add | act | rescan | refresh` (D64, D65) |
