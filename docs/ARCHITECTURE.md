...
| fullscreen | A window going fullscreen takes a workspace of its own, as a macOS app does: `Windows` reconciles the compositor's client list on every `fullscreen` event and once at start, moves a fullscreen window that shares a workspace with others to an empty one (`window.move({ workspace = "empty", follow = true })`), remembers where it came from, and moves it back when it leaves fullscreen. One already alone stays; the alt-tab between it and the desktop is then a workspace switch, so the desktop's windows are never under it and it is never under them (D48) |
