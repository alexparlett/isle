...
| drives | `Drives` fronts the cloud storage services, one provider per service over rclone (`shell/drives`, `~/.config/isle/drives/<name>/provider.json`), each manifest naming the rclone backend it is and all running `scripts/drives.py`, so another service is a manifest rather than code. Settings › Drives lists the accounts, not the services: Add account opens a sheet whose fields come from `rclone config providers`, so Proton asks for a username, password and 2FA while Google, Dropbox and OneDrive are a name and a consent page in a terminal. An account is mounted under `~/Drives/<name>` by `isle-drive@<name>.service`, a user unit, so a mount outlives the shell and comes back at every login; the launcher's file search leaves that root alone, since walking a network mount hangs it. IPC `drives status | services | add | act | rescan | refresh` (D64) |
