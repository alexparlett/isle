...
| shell updates | `IsleUpdate` fetches the checkout's origin hourly and counts the commits behind; Update pulls fast-forward and runs `install.sh` as a process whose lines are kept, the phase and the last line on the row, and Quickshell reloads from the changed files. An SSH remote the host does not know gets a row pointing at the key in Keychain; an https remote gets a gh sign-in. Settings › Updates puts it above the package list; the island's download glyph and the panel footer count cover both |
