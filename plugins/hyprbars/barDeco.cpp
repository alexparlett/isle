...
static bool wantsOwnDecorations(const PHLWINDOW& w) {
    ...
    if (const auto SURF = w->m_xdgSurface.lock()) {
        if (const auto TL = SURF->m_toplevel.lock(); TL && TL->m_resource) {
            ...
            const auto  IT    = DECOS.find(TL->m_resource->resource());
            if (IT != DECOS.end() && IT->second)
                return IT->second->mostRecentlyRequested == ZXDG_TOPLEVEL_DECORATION_V1_MODE_CLIENT_SIDE;
        }
    }
    if (const auto RES = w->resource(); RES && PROTO::serverDecorationKDE) {
        for (const auto& d : PROTO::serverDecorationKDE->m_decos) {
            if (d && d->m_surf == RES)
                return d->m_mostRecentlyRequested == ORG_KDE_KWIN_SERVER_DECORATION_MANAGER_MODE_CLIENT;
        }
    }
    return false;
}
