...
Glass {
    ...
    border.color: editing ? Theme.accent : Theme.hairline
    border.width: editing ? 1 : 1
    z: dragging ? 5 : 0
    ...
    MouseArea {
        ...
        onPressed: mouse => { startX = mouse.x; startY = mouse.y; originX = root.x; originY = root.y; root.dragging = true; }
        onPositionChanged: mouse => { if (!root.dragging) return; root.x = originX + (mouse.x - startX); root.y = originY + (mouse.y - startY); }
        onReleased: {
            root.dragging = false;
